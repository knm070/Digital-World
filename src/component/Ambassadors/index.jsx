  ...
  return (
    <div className='py-[120px] bg-[#FFFFFF]'>
        ...
        <p className='py-[4px] px-[16px] text-[12px] rounded-[16px]' style={{ color : '#413DDE' , border: '1px solid', background:' linear-gradient(90deg, rgba(80, 76, 234, 0.20) 0%, rgba(80, 76, 234, 0.00) 100%), linear-gradient(270deg, rgba(80, 76, 234, 0.20) -1.49%, rgba(80, 76, 234, 0.00) 98.07%), #FFF', boxShadow: '0px 0.636px 0px 0px rgba(255, 255, 255, 0.30) inset'}}>AMBASSADORS</p>
            <h2 className='pt-[16px] text-[48px] sm:text-[28px] md:text-[48px] max-sm:text-[20px] max-sm:w-full max-sm:text-center' style={{fontFamily:'Inter', fontWeight:'700', color:'#000000'}}>Ambassadors</h2>
            <p className='text-[20px] text-center w-[719px] pt-[16px] sm:w-full sm:text-[16px] max-sm:text-[16px] max-sm:w-full' style={{fontFamily:'Inter', fontWeight:'400', color:'#000000CC'}}>We are selecting ambassadors from the top 20 countries in IT business, investment, and education sectors.</p>
        </div>

        <div className='flex justify-center gap-[32px] pt-[64px] flex-wrap'>
            {ambassadors.map((item, index) => {
                return (
                    <div className='w-[375px] h-[480px] object-contain flex flex-col justify-end rounded-[44px] overflow-hidden max-sm:w-full max-sm:mx-[10px]' style={{backgroundImage: `url(${item.image})`,backgroundSize: 'cover',backgroundPosition: 'center',}}>
                        <div className='p-[24px] ' style={{background: '#FFFFFF33',backdropFilter: 'blur(24px)', borderRadius: '24px 24px 0 0 '}}>
                            <div className='flex justify-between items-center'>
                                <h2 className='text-[30px] pb-[16px] max-sm:text-[20px]' style={{fontFamily:'Inter', fontWeight:'600', color:'#FFFFFF'}}>{item.name}</h2>
                                <svg className='max-sm:mb-[20px]' width="24" height="30" viewBox="0 0 24 30" fill="none" xmlns="http://www.w3.org/2000/svg">
